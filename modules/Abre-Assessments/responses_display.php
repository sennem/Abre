<?php
    ...
    */
	...
	if($pagerestrictions=="")
	{
		...
		$sqllookup = "SELECT Title, Grade, Subject FROM assessments WHERE ID='$Assessment_ID'";
		...
		$sqllookup = "SELECT ID FROM assessments_questions WHERE Assessment_ID='$Assessment_ID' ORDER BY Question_Order, ID";
		$result2 = $db->query($sqllookup);
		while($row = $result2->fetch_assoc())
		{
			$Question_ID=htmlspecialchars($row["ID"], ENT_QUOTES);

			$sqllookup = "SELECT Student_FirstName, Student_Lastname FROM assessments_answers where Assessment_ID='$Assessment_ID' and Question_ID='$Question_ID'";
			$result2 = $db->query($sqllookup);
			while($row = $result2->fetch_assoc())
			{
