<?php
    ...
    */
	...
	$sqlstartsession = "SELECT COUNT(*) FROM assessments_status WHERE User='".$_SESSION['useremail']."' AND Assessment_ID='$Assessment_ID'";
	$resultstartsession = $db->query($sqlstartsession);
	$returnrow = $resultstartsession->fetch_assoc();
	$sessionstart = $returnrow["COUNT(*)"];
	if($sessionstart!=1)
	{
		//Check if Assessment actually exists
		$sqlstartecheck = "SELECT COUNT(*) FROM assessments WHERE ID='$Assessment_ID' AND Session_ID='$Session_ID'";
		$resultstartcheck = $db->query($sqlstartecheck);
		$returnrow = $resultstartcheck->fetch_assoc();
		$sessionstartcheck = $returnrow["COUNT(*)"];
	...
	$query = "SELECT Bank_ID FROM assessments_questions WHERE Assessment_ID='$Assessment_ID' ORDER BY Question_Order LIMIT 1";
	...
	$sqlstartsessionturnin = "SELECT COUNT(*) FROM assessments_status WHERE User='".$_SESSION['useremail']."' AND Assessment_ID='$Assessment_ID' AND End_Time!='0000-00-00 00:00:00'";
	$resultstartsessionturnin = $db->query($sqlstartsessionturnin);
	$returnrow = $resultstartsessionturnin->fetch_assoc();
	$sessionstartturnin = $returnrow["COUNT(*)"];
	...
	else
	{
		...
		if($firstquestioncount!=0)
		{
			...
			$sqllookup = "SELECT Title FROM assessments WHERE ID='$Assessment_ID' AND Session_ID='$Session_ID'";
			...
			while($row = $result->fetch_assoc())
			{
					...
					echo "<div style='position: absolute; top:0; bottom:0; left:0; right:0; overflow-y: hidden;'>";
						...
						echo "<div id='assessmentquestions' style='position: absolute; top:0; bottom:0; width:300px; overflow-y: scroll; background-color:"; echo getSiteColor(); echo ";'>";
							$query = "SELECT ID, Bank_ID, Points FROM assessments_questions WHERE Assessment_ID='$Assessment_ID' ORDER BY Question_Order";
							$dbreturn = databasequery($query);
							...
							foreach ($dbreturn as $value)
							{
								...
								echo "<div class='question pointer' id='questionbutton-$questioncount' data-bankid='$Bank_ID' data-questionnumber='$questioncount' style='padding:2px 30px 2px 30px;'>";
									echo "<div style='float:left; padding:12px 20px 0 0;' id='questionicon-$Bank_ID'>";
										...
										$queryanswer = "SELECT COUNT(*) FROM assessments_scores WHERE ItemID='$Bank_ID' AND User='".$_SESSION['useremail']."' AND Assessment_ID='$Assessment_ID'";
										$dbreturnanswer = $db->query($queryanswer);
										$returnrow = $dbreturnanswer->fetch_assoc();
										$answered = $returnrow["COUNT(*)"];
										if($answered==0)
										{
