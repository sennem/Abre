    ...
    */
	...
	if($pagerestrictions=="")
	{
		...
		echo "<div class='row' style='padding:15px;'>";
			...
			$sql = "SELECT Points, Type FROM assessments_questions WHERE Assessment_ID='$assessmentid' AND Bank_ID='$questionid'";
			$result = $db->query($sql);
			while($row = $result->fetch_assoc())
			{
				...
				if($QuestionType=="Open Response")
				{
					...
					$CurrentScore="";
					$sql2 = "SELECT Score FROM assessments_scores WHERE Assessment_ID='$assessmentid' AND ItemID='$questionid' AND User='$user'";
		...
		<script>
			...
			$(function()
			{
				...
				<?php
				$query = "SELECT Response FROM assessments_scores WHERE User='$user' AND ItemID='$questionid' AND Assessment_ID='$assessmentid'";
