...
<script>
	...
	$(function(){
		...
		function SearchQuery(page, sort, method){
			...
			$.post( "modules/<?php echo basename(__DIR__); ?>/closed_display.php",
		...
		//Paging Button
		$("#searchresultsloader").off('.pagebuttonclosed').on('click', '.pagebuttonclosed', function(){
			event.preventDefault();
		...
		//Sort Button
		$("#searchresultsloader").off('.sortbuttonclosed').on('click', '.sortbuttonclosed', function(){
			event.preventDefault();
			var Sort = $(this).data('sort');
			var SortMethod = $(this).data('sortmethod');
			$('.mdl-layout__content').animate({scrollTop:0}, 0);
