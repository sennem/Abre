<?php
    ...
    */
	...
	echo "<div class='widgetsort'>";
		...
		$sql = "SELECT widgets_order, widgets_hidden FROM profiles WHERE email = '".$_SESSION['useremail']."'";
		$result = $db->query($sql);
		$widgets_order = NULL;
		$widgets_hidden = NULL;
...
<script>
	...
	$(function(){
		...
		//Reload Widget
		$('body').on('click', '.widget_reload', function(event){
	  	...
	  	//Make Div Clickable
		$('body').on('click', '.widget_holder_link', function(event){
			event.preventDefault();
			var BodyDiv = $(this).closest(".collapsible-body");
			var Path = $(this).data('path');
