	...
	if(admin() or conductAdminCheck($_SESSION['useremail']) or conductMonitor($_SESSION['useremail'])){
		...
		foreach ($dbreturn as $value){
			...
			if($resultcounter == 0){
				...
				echo "<th class='sortbuttonverification' data-sort='Submission_Time' data-sortmethod='$SortMethod' width='250px'>Submission</th>";
				echo "<th class='sortbuttonverification' data-sort='Student_LastName' data-sortmethod='$SortMethod'>Student</th>";
				echo "<th class='sortbuttonverification' data-sort='Offence' data-sortmethod='$SortMethod'>Offense</th>";
				echo "<th class='sortbuttonverification' data-sort='Owner_Name' data-sortmethod='$SortMethod'>Submitter</th>";
				echo "</tr>";
				echo "</thead>";
				echo "<tbody>";
			}
			...
			if($totalresults == $resultcounter){
				...
				if($totalpossibleresults > $PerPage){
					...
					echo "<ul class='pagination center-align'>";
						if($PageNumber != 1){ echo "<li class='pagebuttonverification' data-page='$previouspage' data-sort='$PageSort' data-sortmethod='$SortMethod'><a href='#'><i class='material-icons'>chevron_left</i></a></li>"; }
						for ($x = $pagingstart; $x <= $pagingend; $x++){
							if($PageNumber == $x){
								echo "<li class='active pagebuttonverification' style='background-color: ".getSiteColor().";' data-page='$x'><a href='#'>$x</a></li>";
							}else{
								echo "<li class='waves-effect pagebuttonverification' data-page='$x' data-sort='$PageSort' data-sortmethod='$SortMethod'><a href='#'>$x</a></li>";
							}
						}
						if($PageNumber != $totalpages){ echo "<li class='waves-effect pagebuttonverification' data-page='$nextpage' data-sort='$PageSort' data-sortmethod='$SortMethod'><a href='#'><i class='material-icons'>chevron_right</i></a></li>"; }
					echo "</ul>";
					echo "</div>";
				}
				echo "</div>";
		...
		if($totalresults == 0){
			echo "<div style='padding:56px; text-align:center; width:100%;'><span style='font-size: 22px; font-weight:700'>No Verification Incidents</span><br><p style='font-size:16px; margin:20px 0 0 0;'>Click the '+' in the bottom right to create an incident.</p></div>";
		}
...
<script>
	...
	$(function(){
		...
		$( ".incidentdetails" ).unbind().click(function(){
			...
			<?php if(conductMonitor($_SESSION['useremail'])){ ?>
							if(Type === "Office"){
								$("#personal").prop("disabled", true);
							}else{
								$("#office").prop("disabled", true);
							}
							$("#Offence").prop("readonly", true);
							$("#Location").prop("readonly", true);
							$("#Description").prop("readonly", true);
							$("#Information").prop("readonly", true);
							$("#incidentDateDiv").hide();
							$("#incidentTimeDiv").hide();
