    ...
    */
	...
	if($pagerestrictions=="")
	{
		...
		echo "<div class='mdl-shadow--2dp' style='background-color:#fff; padding:20px 40px 40px 40px'>";
			...
			echo "<h5>ELL Students</h5>";

			echo "<table id='table3' class='tablesorter bordered highlight'>";
			...
			echo "<tbody>";
					...
					if($CounselingID != ""){
						$lastStudentID = "";
						$query = "SELECT Abre_Student_Counselors.StudentId, Abre_Students.FirstName, Abre_Students.LastName FROM Abre_Student_Counselors LEFT JOIN Abre_Students ON Abre_Student_Counselors.StudentId = Abre_Students.StudentId WHERE Abre_Student_Counselors.CounselorStaffId = '$StaffId' AND Abre_Students.ELL != 'N' ORDER BY Abre_Student_Counselors.StudentId";
						$dbreturn = databasequery($query);
						foreach($dbreturn as $value){
							$StudentId = htmlspecialchars($value["StudentId"], ENT_QUOTES);
							if($StudentId == $lastStudentID){
