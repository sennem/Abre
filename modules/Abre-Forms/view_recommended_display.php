<?php
    ...
    */
	...
	require_once('../../core/abre_functions.php');
		...
		if($searchquery == ""){
			$querycount = "SELECT COUNT(*) FROM forms WHERE Public = '1' AND (Restrictions = '' OR Restrictions LIKE '%$usertype%')";
			$sql = "SELECT ID, Name FROM forms WHERE Public = '1' AND (Restrictions = '' OR Restrictions LIKE '%$usertype%') LIMIT $LowerBound, $PerPage";
		}else{
			$querycount = "SELECT COUNT(*) FROM forms WHERE Public = '1' AND (Restrictions = '' OR Restrictions LIKE '%$usertype%') AND NAME LIKE '%$searchquery%'";
			$sql = "SELECT ID, Name FROM forms WHERE Public = '1' AND (Restrictions = '' OR Restrictions LIKE '%$usertype%') AND NAME LIKE '%$searchquery%' LIMIT $LowerBound, $PerPage";
		}
		...
		while($row = $result->fetch_assoc()){
			...
			echo "<tr class='formsrow pointer'>";
					...
					echo "<td width='70px' class='hide-on-med-and-down exploreform' data-href='#forms/view/$ID'>";
					...
					echo "</td>";
					echo "<td class='exploreform' data-href='#forms/view/$ID'>$Name</td>";
			echo "</tr>";
		}
