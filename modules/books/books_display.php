<?php 
    ...
    */	
	...
	if($pagerestrictions=="")
	{
		...
		<div class='row'><div class='col s12'><h5>My Library</h5></div></div>
		<div class='row'>
		<?php
		
		$userid=finduseridcore($_SESSION['useremail']);
		$sql = "SELECT * FROM books_libraries where User_ID='$userid' order by ID DESC";
		$result = $db->query($sql);
		...
		if($numrows==0)
		{
			echo "<div class='col s12'>Click the '+' in the bottom right to add a book to your library.</div>";
		}
		?>
