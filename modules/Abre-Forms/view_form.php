<?php
    ...
    */
	...
	if(isset($_GET["sessionid"])){ $sessionid=htmlspecialchars($_GET["sessionid"], ENT_QUOTES); }else{ $sessionid=""; }
	...
	if($restrictions != ""){
		if (strpos($restrictions, $_SESSION['usertype']) === false){ $access=0; }
	}
	...
	if($access==1)
	{
		...
		echo "<div class='page'>";
			...
			if($sessionid == ""){
				$sqlcheck = "SELECT COUNT(*) FROM forms WHERE ID = '$id'";
			}else{
				$sqlcheck = "SELECT COUNT(*) FROM forms WHERE ID = '$id' AND Session = '$sessionid'";
			}
			$resultcheck = $db->query($sqlcheck);
			$dbreturnpossible = $resultcheck->fetch_assoc();
			if($dbreturnpossible['COUNT(*)'] > 0){
			...
			if($dbreturnpossible==0){
				echo "<div class='row' style='padding:56px; text-align:center; width:100%;'><span style='font-size: 22px; font-weight:700'>This form does not exist</span><br><p style='font-size:16px; margin:20px 0 0 0;'>Make sure that you have the correct URL.</p></div>";
			}
