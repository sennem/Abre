<?php
    ...
    */
	...
	if(admin() || isFormsAdministrator())
	{
		...
		if($searchquery == ""){
			$querycount = "SELECT COUNT(*) FROM forms WHERE Template ='1'";
			$sql = "SELECT ID, Name, Owner, LastModified FROM forms WHERE Template ='1' LIMIT $LowerBound, $PerPage";
		}else{
			$querycount = "SELECT COUNT(*) FROM forms WHERE (LOWER(Name) LIKE '%$searchquery%') AND Template ='1'";
			$sql = "SELECT ID, Name, Owner, LastModified FROM forms WHERE (LOWER(Name) LIKE '%$searchquery%') AND Template ='1' LIMIT $LowerBound, $PerPage";
		}
		...
		while($row = $result->fetch_assoc()){
			...
			echo "<tr class='formsrow pointer'>";
					...
					echo "<td width='70px' class='templatepreview' data-formid='$ID' data-formname='$Name'>";
					...
					echo "</td>";
					echo "<td class='templatepreview' data-formid='$ID' data-formname='$Name'>$Name</td>";
					echo "<td class='hide-on-med-and-down templatepreview' data-formid='$ID' data-formname='$Name'>$DisplayName</td>";
					echo "<td class='hide-on-med-and-down templatepreview' data-formid='$ID' data-formname='$Name'>$LastModified</td>";
		...
		if($totalformscount == 0){
			echo "<div class='row' style='padding:56px; text-align:center; width:100%;'><span style='font-size: 22px; font-weight:700'>No Templates Found</span><br><p style='font-size:16px; margin:20px 0 0 0;'>There are no templates matching this search criteria.</p></div>";
		}
