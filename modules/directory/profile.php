	...
	{	
		...
		echo "<div class='page'>";
			...
			else
			{
				...
				if($picture==""){ 
					$picture='user.png'; 
					$pictureserver=$portal_root."/modules/directory/serveimage.php?file=$picture&ext=.png";
				}
				else
				{
		...
		<script>
			...
			$(function() {
				...
				$("#archiveuser").click(function(event){
					...
					$(formMessages).text('Archiving the employee...');	
				    $( ".notification" ).slideDown();
				    $.ajax({
					    type: 'POST',
				...
				$('#disciplinediv').on('click','.deletedisciplinerecord',function(event){
					...
					$(formMessages).text('Deleting the discipline record...');	
				    $( ".notification" ).slideDown();
				    $.ajax({
					    type: 'POST',
