    ...
    */
	...
	if(CONSTANT('SITE_MODE') == "DEMO"){
		echo "<div style='padding:30px; text-align:center; width:100%;'>";
			echo "<div class='row'>";
				echo "<span style='font-size: 22px; font-weight:700'>Learn more about the Conduct App!</span>";
			echo "</div>";
			echo "<div class='row'>";
...
<script>
	...
	$(function(){
		...
		function SearchQuery(page, sort, method){
			...
			$.post( "modules/<?php echo basename(__DIR__); ?>/open_display.php",
		...
		//Paging Button
		$('#searchresultsloader').off('.pagebuttonopen').on('click', '.pagebuttonopen', function(){
			event.preventDefault();
		...
		//Sort Button
		$('#searchresultsloader').off('.sortbuttonopen').on('click', '.sortbuttonopen', function(){
			event.preventDefault();
			var Sort = $(this).data('sort');
			var SortMethod = $(this).data('sortmethod');
			$('.mdl-layout__content').animate({scrollTop:0}, 0);
