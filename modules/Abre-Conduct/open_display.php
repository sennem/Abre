    ...
    */
	...
	foreach($dbreturn as $value){
		if($resultcounter == 0){
			...
			echo "<th class='sortbuttonopen' data-sort='Submission_Time' data-sortmethod='$SortMethod' width='250px'>Submission</th>";
			echo "<th class='sortbuttonopen' data-sort='Student_LastName' data-sortmethod='$SortMethod'>Student</th>";
			echo "<th class='sortbuttonopen' data-sort='Building' data-sortmethod='$SortMethod'>Building</th>";
			echo "<th class='sortbuttonopen' data-sort='Type' data-sortmethod='$SortMethod'>Type</th>";
			echo "</tr>";
			echo "</thead>";
			echo "<tbody>";
		}
		...
		echo "<tr class='incidentdetails pointer' data-studentname='$Student_FirstName $Student_LastName' data-type='$Type' data-offence='$Offence' data-location='$Location' data-description='$Description' data-information='$Information' data-incidentdate='$Incident_Date' data-incidenttime='$Incident_Time' data-served='$Consequence_Served' data-consequence='$Consequence' data-studentcode='$SchoolCode' data-studentbuilding='$Building' data-studentid='$StudentID' data-submissionid='$SubmissionID' data-servedate='$Serve_Date' data-thrudate='$Thru_Date' data-totaldays='$Total_Days' data-consequenceid='$Consequence_ID' data-firstname='$Student_FirstName' data-lastname='$Student_LastName' data-middlename='$Student_MiddleName' data-dupincidentid = '$dupIncidentId' data-reload='open'>";
		...
		if($totalresults == $resultcounter){
			...
			if($totalpossibleresults > $PerPage){
				...
				echo "<ul class='pagination center-align'>";
					if($PageNumber != 1){ echo "<li class='pagebuttonopen' data-page='$previouspage' data-sort='$PageSort' data-sortmethod='$SortMethod'><a href='#'><i class='material-icons'>chevron_left</i></a></li>"; }
					for($x = $pagingstart; $x <= $pagingend; $x++){
						if($PageNumber == $x){
							echo "<li class='active pagebuttonopen' style='background-color: ".getSiteColor().";' data-page='$x'><a href='#'>$x</a></li>";
						}else{
							echo "<li class='waves-effect pagebuttonopen' data-page='$x' data-sort='$PageSort' data-sortmethod='$SortMethod'><a href='#'>$x</a></li>";
						}
					}
					if($PageNumber != $totalpages){ echo "<li class='waves-effect pagebuttonopen' data-page='$nextpage' data-sort='$PageSort' data-sortmethod='$SortMethod'><a href='#'><i class='material-icons'>chevron_right</i></a></li>"; }
				echo "</ul>";
				echo "</div>";
			}
			echo "</div>";
	...
	if($totalresults == 0){
		if($ConductSearch == ""){
			echo "<div style='padding:56px; text-align:center; width:100%;'><span style='font-size: 22px; font-weight:700'>No Open Incidents</span><br><p style='font-size:16px; margin:20px 0 0 0;'>Click the '+' in the bottom right to create an incident.</p></div>";
		}else{
			echo "<div style='padding:56px; text-align:center; width:100%;'><span style='font-size: 22px; font-weight:700'>No Open Incidents Found</span><br><p style='font-size:16px; margin:20px 0 0 0;'>Click the '+' in the bottom right to create an incident.</p></div>";
		}
	}
...
<script>
	...
	$(function(){
		...
		$( ".incidentdetails" ).unbind().click(function(){
			...
			if(Type === "Personal"){ $('#personal').prop('checked',true); $('#archiveIncident').show(); }
			...
			if(Type === "Office"){
				...
				$("#conduct_footer").hide();
				$("#setTimeButton").hide();
				$(".removeConsequencesButton").hide();
				$("#addconsequencebutton").hide();
				$(".servedCheckbox").prop("disabled", true);
